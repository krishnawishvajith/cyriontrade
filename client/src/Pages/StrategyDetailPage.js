...
const StrategyDetailPage = () => {
  ...
  useEffect(() => {
    // Find the item from all marketplace categories
    let foundItem = null;
    
    for (const category in MARKETPLACE_ITEMS) {
      const items = MARKETPLACE_ITEMS[category];
      ...
      if (foundItem) {
        ...
        const descriptionData = STRATEGY_DESCRIPTIONS[id] || {
          overview: STRATEGY_DESCRIPTIONS.default(foundItem.title, category)
        };
        
        // Add additional data for the detail page
        setStrategy({
          ...foundItem,
          type: category === "Strategies" ? "Strategy" : category.slice(0, -1), // Remove 's' from category
          rating: foundItem.rating || 5,
          reviews: foundItem.reviews || 0,
          image: foundItem.image || null,
          banner: foundItem.banner || foundItem.image || null, // Use banner if available, otherwise use image
        ...
        <div className={classes.layout}>
          ...
          <div className={classes.sidebar}>
            <div className={classes.sidebarHeader}>
              <div className={classes.sidebarTitle}>{strategy.title}</div>
          ...
          <div className={classes.mainContent}>
            <div className={classes.banner}>
              {strategy.banner ? (
                <img src={strategy.banner} alt={strategy.title} />
              ) : (
                <span style={{ fontSize: "6rem" }}>{strategy.emoji}</span>
              )}
            </div>
                ...
                <div>
                  ...
                  {strategy.badge && (
                    <div className={classes.badge}>
                      {strategy.badge === "HIGH WIN RATE" && <TrendingUp style={{ fontSize: "14px" }} />}
                      {strategy.badge === "AI CONFIRMATION" && <VerifiedUser style={{ fontSize: "14px" }} />}
                      {strategy.badge === "POPULAR" && <Star style={{ fontSize: "14px" }} />}
                      {strategy.badge}
                    </div>
                  )}
